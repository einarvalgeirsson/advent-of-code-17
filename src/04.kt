...
private val input = """sayndz zfxlkl attjtww cti sokkmty brx fhh suelqbp
...
private fun part_1(): Int {
    return input
            .split("\n")
            .map { it.split(" ") }
            .filter { it.hasUniqueValues() }
            .count()
}

fun List<String>.hasUniqueValues(): Boolean {
...
private fun part_2(): Int {
    return input
            .split("\n")
            .map { it.split(" ") }
            .filterNot { it.hasAnagrams() }
            .count()
}

fun List<String>.hasAnagrams(): Boolean {
   return this.map { it.toCharArray().sorted() }.distinct().count() < this.count()
}